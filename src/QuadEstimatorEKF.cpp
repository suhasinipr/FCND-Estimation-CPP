...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
 ...
 // Quaternion<float> dq;
  //dq = dq.FromEuler123_RPY(gyro.x, gyro.y, gyro.z);

  //Quaternion<float> q_bar = q * dq;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) = predictedState(5) + ((accel_I.z - 9.81) *dt);
  predictedState(4) = predictedState(4) + accel_I.y*dt;
  predictedState(3) = predictedState(3) + accel_I.x*dt;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  VectorXf RbgPrime_row1(3);
  VectorXf RbgPrime_row2(3);
  VectorXf RbgPrime_row3(3);
  VectorXf u_t(3);
  int i = 0;
  for (i = 0; i < 3; i++)
  {
	  RbgPrime_row1(i) = RbgPrime(0, i);
	  RbgPrime_row2(i) = RbgPrime(1, i); 
	  RbgPrime_row3(i) = RbgPrime(2, i);
  }
  u_t(0) = accel.x;
  u_t(1) = accel.y;
  u_t(2) = accel.z;
  ...
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  float a = u_t.dot(RbgPrime_row1);
  gPrime(3, 6) = u_t.row(0).dot(RbgPrime_row1.row(0)) * dt;
  gPrime(4, 6) = u_t.dot(RbgPrime_row2) * dt;
  gPrime(5, 6) = u_t.dot(RbgPrime_row3) * dt;
